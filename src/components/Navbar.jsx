...
import { ArrowRight, Sun, Moon, X, TextAlignEnd } from "lucide-react";
      ...
      <nav className={`w-full whitespace-nowrap shrink-0 grid-cols-6 fixed flex px-5 lg:px-10 xl:px-[10%] py-6 justify-between items-center z-50 transition-all duration-300 ${isScrolled ? "bg-white/90 dark:bg-gray-950/90 shadow-md" : ""}`}>
        ...
        <div className="flex items-center gap-3 ml-auto">
          <Button 
            ...
            className="flex w-10 h-10 bg-transparent hover:bg-transparent"
          >
            {isDarkMode ? (
              <Moon className="cursor-pointer text-gray-900 dark:text-yellow-50" size={40} />
            ) : (
              <Sun className="cursor-pointer text-gray-900" size={40} />
            )}
          </Button>
          <div className="hidden lg:flex ml-auto px-4 md:px-6 py-1.5 border-gray-500 dark:border-gray-400 rounded-full border text-gray-900 dark:text-gray-100 hover:bg-gray-800 hover:text-white hover:border-white hover:transition-all hover:duration-300">
